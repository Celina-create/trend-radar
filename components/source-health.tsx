...
export function SourceHealthBar({ health, totalPosts, generatedAt }: Props) {
  const okCount = health.filter((h) => h.ok).length
  const time = new Date(generatedAt).toLocaleString('en-US', {
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
    minute: '2-digit',
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-text-tertiary">
            · {okCount}/{health.length} healthy · {totalPosts} posts
          </span>
        </div>
